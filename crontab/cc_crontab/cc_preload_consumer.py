...
from mongo_db import preload_task_db, conn_mongo_nsq_err_log
...
hosts = nsq_config
BASE_MAX_TRIES = 5
nsq_name = 'cdn_%s_preload' % OPT.lower()
...
def process_message(message):
    ...
    try:
        ...
        if nsq_message:
            ...
            refresh_result = CCAPI.domain_preload(args)
            ...
            preload_task = preload_task_db()

            search_sql = {'flag': task_flag}
            mlog = preload_task.find_one(search_sql)
            ...
            if mlog:
                ...
                refresh_info = mlog.get('preload_info', {})
                refresh_info[OPT] = {'task_id': task_id}
                mlog['preload_info'] = refresh_info

                pid = preload_task.update_one(search_sql,  {'$set': mlog})
                if pid:
                    exe_flag = True
